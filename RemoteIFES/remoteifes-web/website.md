...
> **Atenção:** se você abrir a pasta **raiz** do repositório (`RemoteIFES-main`)
> no VS Code em vez desta pasta (`remoteifes-web`), o Live Server passa a
> observar também `remoteifes-server/data/remoteifes.db` — que muda a cada
> login — e recarrega a página sozinho, derrubando a sessão. O arquivo
> `.vscode/settings.json` na raiz já evita isso, mas o mais simples é sempre
> abrir `remoteifes-web` como pasta do VS Code.
