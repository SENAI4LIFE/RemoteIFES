...
const GRUPOS = {
  gestao: { rotulo: "Gestão", subs: ["usuarios", "proprietarios", "relatos"] },
  dispositivos: { rotulo: "Dispositivos", subs: ["macs", "dispositivos", "notificacoes", "esp32"] },
  monitoramento: { rotulo: "Monitoramento", subs: ["ativos", "mapa", "monitoramento"] },
  sistema: { rotulo: "Sistema", subs: ["sessoes", "logs", "acessos", "config", "auditoria"] },
};
...
test("a Administração tem quatro grupos, sem grupo vazio e sem função duplicada", async ({ page, context }) => {
  await abrirAdmin(page, context, "superadmin");
  const grupos = await estrutura(page);

  expect(grupos.map((g) => g.rotulo)).toEqual(["Gestão", "Dispositivos", "Monitoramento", "Sistema"]);
  grupos.forEach((g) => {
...
test("os rótulos antigos de Administração não aparecem mais na navegação", async ({ page, context }) => {
  ...
  for (const obsoleto of ["ESP32", "ESP32 / MACs", "Dispositivos", "Notificações de dispositivos", "Monitoramento"]) {
...
test("o admin comum vê os quatro grupos apenas com as funções que seu nível autoriza", async ({ page, context }) => {
...
for (const nome of ["mobile-compact", "mobile-portrait", "mobile-landscape", "tablet-portrait", "notebook", "desktop"]) {
  test(`a navegação agrupada cabe e permanece alcançável em ${nome}`, async ({ page, context }) => {
    ...
    expect(medidas.grupos.map((g) => g.texto)).toEqual(["Gestão", "Dispositivos", "Monitoramento", "Sistema"]);
