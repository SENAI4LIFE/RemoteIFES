const Admin = {
    ...
    usuarios.forEach((u) => {
      ...
      li.innerHTML = `
        <div>
          ...
          <div class="room-sub senha-area hidden" data-senha-de="${u.id}"></div>
        </div>
        <div class="agenda-actions">
          <button type="button" class="link-btn ver-senha">ver senha</button>
          <button type="button" class="link-btn trocar-senha">trocar senha</button>
      ...
      li.querySelector(".ver-senha").addEventListener("click", async () => {
        const area = li.querySelector(".senha-area");
        if (!area.classList.contains("hidden")) {
          area.classList.add("hidden");
          return;
        }
        const resp = await Api.verSenhaUsuario(u.id);
        area.textContent = resp.ok ? `senha atual: ${resp.senha}` : resp.erro;
    ...
    usuarios.forEach((u) => {
      ...
      li.innerHTML = `
        <div>
          ...
          <div class="room-sub">@${u.usuario} · último acesso: ${u.ultimoAcesso}</div>
        </div>
        <span class="status-badge ${u.online ? "on" : "off"}">${u.online ? "online" : "offline"}</span>
      `;
      list.appendChild(li);
    });
  },
  ...
  async carregarSessoes(data) {
    ...
    sessoes.forEach((s) => {
      ...
      li.innerHTML = `
        <div>
          <div class="room-name">${s.nome} (@${s.usuario})</div>
          <div class="room-sub">login: ${s.login} · logout: ${s.logout || "ainda conectado"}</div>
        </div>
      `;
      list.appendChild(li);
    });
    ...
    eventos.forEach((e) => {
      ...
      li.innerHTML = `
        <div>
          ...
          <div class="room-sub">${e.status === "online" ? "ficou online" : "ficou offline"} · ${e.criadoEm}</div>
        </div>
  ...
  async carregarMapa() {
    ...
    salas.forEach((s) => {
      ...
      div.title = `${s.sala} — ${s.online ? (s.ligado ? "online, ligado" : "online, desligado") : "offline"}${s.agendadaAgora ? " · reservada agora" : ""}`;
...
document.getElementById("criarUsuarioBtn").addEventListener("click", async () => {
  ...
  const dados = {
    ...
    isAdmin: document.getElementById("novoUsuarioAdmin").checked,
  };
