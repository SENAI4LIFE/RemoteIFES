...
router.patch("/admin/usuarios/:id/login", (req, res) => {
  try {
    const atualizado = usuariosService.trocarLogin(Number(req.params.id), req.body.novoLogin, req.usuario);
    res.json({ ok: true, usuario: atualizado });
  } catch (err) {
    res.status(400).json({ ok: false, erro: err.message });
  }
...
router.get("/admin/usuarios/:id/senha", (req, res) => {
  try {
    res.json({ ok: true, ...usuariosService.obterSenha(Number(req.params.id), req.usuario) });
  } catch (err) {
    res.status(400).json({ ok: false, erro: err.message });
  }
});
...
router.patch("/admin/usuarios/:id/senha", (req, res) => {
  try {
    usuariosService.trocarSenha(Number(req.params.id), req.body.novaSenha, req.usuario);
    res.json({ ok: true });
  } catch (err) {
    res.status(400).json({ ok: false, erro: err.message });
  }
