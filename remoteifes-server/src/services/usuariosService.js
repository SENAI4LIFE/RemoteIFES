...
function obterSenha(id, requisitante) {
  const usuario = buscarPorId(id);
  if (!usuario) throw new Error("usuário não encontrado");
  if (usuario.nivel === NIVEL_SUPERADMIN && (!requisitante || requisitante.nivel !== NIVEL_SUPERADMIN)) {
    throw new Error("apenas o administrador principal pode visualizar a própria senha");
  }
  return { usuario: usuario.usuario, senha: usuario.senha };
}
