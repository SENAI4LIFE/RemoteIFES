...
module.exports = [
  ...
  },
  {
    ...
    corpo: [
      { t: "comando", titulo: "Deploy e rollback protegidos", comandos: C.deploy.slice(0, 5), quando: "Deploy de origin/main/tag e retorno manual à versão anterior ou ref conhecido.", preRequisitos: "Clone de produção; main sem mudanças locais; serviço instalado; dados persistentes; ref já local para --offline.", resultado: "Deploy cria pre-update, resolve o ref, instala dependências se mudaram, reinicia e exige que o processo em execução responda saudável ao /health informando o commit implantado (um processo antigo que sobreviveu a um restart falho não conta); falha aciona rollback automático. Rollback cria pre-rollback e exige a mesma confirmação do commit alvo. Uma versão alvo anterior ao campo commit só é aceita se o /health mostrar um processo que subiu depois do restart, registrada como identidade não confirmada. Se o código já está na versão pedida, o script ainda consulta o processo em execução e reinicia quando ele não a confirma.", risco: "--force permite descartar mudanças locais. Rollback troca código, não desfaz esquema do banco; restaure o pre-update compatível se a versão antiga não abrir o banco migrado." },
