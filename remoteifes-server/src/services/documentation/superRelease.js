...
module.exports = [
  {
    ...
    corpo: [
      { t: "comando", titulo: "Preparar o projeto (uma vez por checkout)", comandos: C.androidPreparo, quando: "Antes do primeiro build e sempre que package.json ou config.xml mudarem.", preRequisitos: "Node e npm; validate não exige o Android SDK.", resultado: "Dependências instaladas e configuração validada (esquemas, permissões, origem).", risco: "Nenhum." },
      { t: "comando", titulo: "Android: preparar, compilar e executar (desenvolvimento)", comandos: C.androidDesenvolvimento, quando: "Testar o pacote em emulador ou aparelho em modo desenvolvedor; cada linha é uma etapa, não um pré-requisito da seguinte.", preRequisitos: "JDK 17, SDK/API 36, Build Tools 36, Platform Tools, ANDROID_HOME e Gradle 8.14.2; para run, um aparelho ou emulador visível ao adb.", resultado: "www/ é sincronizado do website, a plataforma é preparada e o APK de desenvolvimento é gerado ou executado.", risco: "build-android é de desenvolvimento (debug): nunca publique esse artefato." },
      { t: "comando", titulo: "iOS: preparar, compilar e executar (só em macOS)", comandos: C.iosDesenvolvimento, quando: "Validação em simulador ou aparelho iOS; não há publicação iOS neste projeto.", preRequisitos: "macOS, Xcode, Command Line Tools e CocoaPods.", resultado: "Projeto Xcode preparado, build e execução no simulador ou aparelho.", risco: "Assinatura e distribuição iOS ficam fora do fluxo documentado." },
  ...
  },
  {
    ...
    corpo: [
      { t: "comando", titulo: "Endurecer ou restaurar config.xml", comandos: C.androidRede, quando: "Teste manual da política; build-android-release já faz endurecimento e restauração automaticamente.", preRequisitos: "Execute em remoteifes-cordova e use uma única origem HTTP/HTTPS válida.", resultado: "Produção restringe access/navigation/intent à origem; o esquema da WebView acompanha o da origem (http://localhost para servidor HTTP, https://localhost para HTTPS), HTTPS bloqueia cleartext e HTTP local mantém exceção com aviso. dev-config restaura curingas de desenvolvimento.", risco: "Não distribua APK com configuração curinga. HTTP transmite credenciais e OTA sem criptografia e só cabe em LAN controlada. Uma página https://localhost não alcança servidor HTTP: o Chromium bloqueia fetch e WebSocket como conteúdo misto." },
  ...
  },
  {
    ...
    corpo: [
      { t: "comando", titulo: "Sincronizar com o remoto", comandos: C.gitSincronizar, quando: "Na raiz do repositório: export para adicionar, commitar e enviar; import para atualizar a partir de origin/main.", preRequisitos: "Git instalado, remoto e branch main corretos; árvore e diff revisados; autorização para push.", resultado: "export usa git add -A, cria o commit e envia origin/main; import faz pull.", risco: "export inclui todas as mudanças rastreáveis, inclusive arquivos soltos na raiz: revise o diff antes." },
      { t: "comando", titulo: "Recriar todo o histórico (destrutivo, à parte)", comandos: C.gitRecriar, quando: "Somente para recriar deliberadamente o repositório do zero.", preRequisitos: "Decisão registrada; backup do clone; ninguém dependendo do histórico atual.", resultado: "Cria um histórico órfão e força o remoto após confirmação.", risco: "Apaga permanentemente todo o histórico anterior e faz push -f: nunca em operação normal, deploy ou limpeza local." },
