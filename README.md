...
O agendador do servidor verifica agendamentos ativos a cada minuto e não repete uma mesma ação (ligar/desligar) mais de uma vez no mesmo dia. Um agendamento desativado permanece salvo, mas não é executado; o autor do agendamento ou um administrador podem ativá-lo, desativá-lo ou removê-lo — outros usuários comuns só podem visualizar. Se o servidor parar depois de ligar e só voltar no dia seguinte, o desligamento que ficou pendente é aplicado uma única vez na primeira passagem do agendador, feita ao iniciar e antes de qualquer ESP32 reconectar (a reconexão recebe o OFF, não o "ligado" expirado) — a menos que uma intenção mais nova (comando manual, outro agendamento ou OFF local) tenha surgido depois da hora em que ele era devido; agendamentos desativados não são recuperados e uma execução já registrada nunca se repete.
...
As atualizações que chegam pelo WebSocket ou pelas consultas periódicas (lista e grade de salas, painel da sala, planta baixa, cartões de ESP32, seletores de clonadora, cartões de Status do sistema) são aplicadas **no lugar** (`js/ui-sync.js`): cada sala ou cartão continua sendo o mesmo elemento, na mesma posição, e só os textos, classes e atributos que mudaram são reescritos — a ordem em que os dados chegam nunca reordena o que já está na tela, e o foco do teclado, a rolagem e uma interação em curso sobrevivem a cada mensagem. Só uma mudança estrutural real (sala ou dispositivo adicionado ou removido, filtro ou navegação do usuário) altera o layout. O frontend mantém uma única conexão WebSocket por aba (compartilhada entre a tela de status do servidor e o canal de salas/status), em vez de abrir conexões redundantes. O servidor também limita a quantidade de mensagens que uma conexão autenticada pode enviar em uma janela de tempo curta, encerrando a conexão em caso de flood.
...
remoteifes-web/
  ...
  js/
    ...
    ui-sync.js         atualização em tempo real sem reconstruir o DOM: listas com chave estável e conteúdo reconciliado no lugar
