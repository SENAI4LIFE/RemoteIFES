...
`test-ios` comprova a casca nativa: instalação, cold start, sobrevivência ao terminate/relaunch, início do motor WKWebView pelo cordova-ios e ausência de falhas de carregamento e de relatórios de crash; as capturas ficam em `build/ios-test-*/` (o CI as publica como artefato). Ele **não** inspeciona o conteúdo da página — no iOS não há inspetor scriptável sem ferramentas externas. A correção do frontend no WebKit do iOS é validada pelo smoke do Safari no iOS Simulator (`cd e2e && SAFARI_PLATFORM=ios npm run test:safari`), que usa o mesmo motor da WKWebView. Nenhuma das duas execuções substitui o teste em um iPhone físico, que continua manual.
